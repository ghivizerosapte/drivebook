...
> Последнее обновление: 2026-08-17 (сессия 5)

---

## Сессия 5 — 2026-08-17

**Бренд** (`ed395b6`): публичный бренд везде **PermisPro** — `brand` в
`/v1/meta`, титулы `/book` и `/admin`. JS-API `DriveBookWidget` сохранён
намеренно (внутреннее имя, ломать интеграцию незачем). Admin-акцент выровнен.

**Демо-контент** (`a7e7487`): футер — str. Alba Iulia 75 / +373 60 123 456 /
contact@permispro.md, соцсети и FAB-кнопки правдоподобные. **Всё демо —
заменить на реальные данные перед продом.**

**Mobile Only фиксы**: горизонтальный overflow на 390px — виновник оказался
`.nav-toggle` `margin:-10px` (`811e8f7`); sticky-CTA скрывается над виджетом +
re-anchor при повороте (`46ebc79` + hardening этой сессии, см. ниже).

**Анти-бот v1** (`06ee454`): маска `+373 6x/7x` в виджете, серверная
нормализация/валидация в `POST /v1/bookings`, honeypot (поле переименовано в
`contact_notes_x` после ревью) + min-fill 3 c. Аудит: `rate_limit_buckets`
УЖЕ подключён в `create_booking` (IP 30/мин, phone 10/час) — единственный
лимитированный эндпоинт; `waitlist` phone не валидируется (долг).

**Палитра финальная** (`52a3901`, `ad66baf`): `accent` = точный BSM
`#F9812A`/`#E96607` (снят computed-стилем с bsm.co.uk); текст на оранжевом —
**ЧЁРНЫЙ `#111417`** (решение владельца; у BSM белый — отклонено); `ink`
`#111417`, `muted` нейтральный `#444` без синевы, чекмарки оранжевые. Синие
остались только у outline-кнопки «Cum funcționează» и ссылок. Долг: step-иконки
таймлайна ещё синие.

**Презентация директора**: `landing/presentation.html`, публичный роут
`GET /presentation` (`9bcc61d`), самодостаточный файл ~0.5 МБ. Также
опубликована как Claude Artifact (требует логин — наружу не давать).

**Кросс-модельное ревью**: дифф дня прогнан через Antigravity/Gemini; 6 находок
приняты и исправлены (коммит `fix: harden anti-bot and mobile UX per
cross-model review`), 2 отклонены (`/presentation` публичен намеренно;
`--c-card` определён). Что исправлено:
- re-anchor виджета только на реальную смену ориентации (matchMedia
  portrait↔landscape), а не на каждый resize схлопывания адресной строки;
- honeypot `website` → `contact_notes_x` (autofill предлагал «website» живым
  людям) — виджет + JSON-ключ + Pydantic-модель;
- honeypot: любая непустая строка = спам (пробелы тоже), «» и None легитимны;
- нормализация `+373069123456` → `+37369123456` (снят лишний trunk-0) в API;
- каретка в поле телефона сохраняется при реформатировании;
- sticky-CTA скрывается и когда центр вьюпорта внутри секции (iPhone SE 667px).

**Операционные заметки**: с 20.07 работал stale uvicorn без `--reload` — правки
не попадали в браузер (симптом «ничего не меняется»); слоты протухают через
14 дней после сева — **ПЕРЕД ЛЮБЫМ ДЕМО обязательный `make seed`** (сбрасывает
admin на `admin`/`admin`); сервер сейчас на `0.0.0.0:8100` для теста с телефона
(`192.168.0.25`).

**Следующие шаги**: Приоритет 1 — Railway-деплой (план в §4 прошлой сессии
актуален; после деплоя вписать URL в презентацию, сейчас заглушка
«[ссылка будет здесь]»); Приоритет 2 — supervisor `.env`; Приоритет 3 — долги
...
**Презентация двуязычная**: в шапку `/presentation` добавлен переключатель
RO | RU (пилюли в стиле продукта, переключение мгновенное, выбор хранится в
localStorage). Сделан полный румынский перевод всех текстов (~93 блока) —
живой молдавский румынский с диакритикой ș ț ă. Скриншоты продукта внутри
презентации и так на RO-интерфейсе. **Дефолт — румынский**: пустой
localStorage открывает RO (решение владельца, коммит b2759d2), переключение
на RU запоминается. Коммиты 9348a6d (перевод) + b2759d2 (RO-дефолт). **На
проде**: https://permispro.onrender.com/presentation задеплоено (Manual
Deploy), открывается на румынском, кнопка «Deschide site-ul demo →». Все
поверхности (`/`, `/book`, `/admin`, `/health`) отвечают 200, сервис не спит
(UptimeRobot активен, ответ ~0.2 с).
